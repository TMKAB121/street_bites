...
`favorites.blade.php`, `search.blade.php`, `livewire/**`, `styleguide.blade.php`,
`layouts/**`) alongside reusable
...
| `<x-truck-discovery>` | — (composes others) | The shared discovery section: `<x-truck-filters>` + `<x-location-search>` + `<x-truck-map>` + the `truckDistanceSort` results grid, assembled once so the home page and `/favorites` stay in lockstep. Its Alpine scope owns the active tag. Props: `trucks`, `tags`, `heading`, `empty` |
